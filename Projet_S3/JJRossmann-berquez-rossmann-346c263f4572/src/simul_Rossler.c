...
void simul_Rossler(FILE* fichier , float tpssimulation, float dt, tri coordpos, tri parametres){
	tri vit;
	vit.x = 0;
	vit.y = 0;
	vit.z = 0;
    ...
    while (tps <= tpssimulation){
        tps = tps+dt;
        vitesses = vitesse_Rossler(coordonnees, parametres);
        coordonnees = position_Rossler(coordonnees, dt, vitesses);
        vit.x = vit.x/10;
        vit.y = vit.y/10;
        vit.z = vit.z/10;
